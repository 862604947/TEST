...
def fill_np_matrix(vects, batch_size, dummy):  # 把不规矩的二维链表vects转化为batch_size×max_len的矩阵返回，实际是进行用dummy编号填充补齐
...
def fill_np_array(vect, batch_size, dummy):   # 用dummy编号补齐一维链表vect到batch_size
...
def batch_train_data(batch_size):
    ...
    with codecs.open(train_path, 'r', 'utf-8') as fin:
        ...
        while not stop:
            ...
            for i in range(batch_size):  # 处理一个批量数据
                line = fin.readline()
                if not line:
                    stop = True
                    break
                    ...
                    toks = line.strip().split('\t')
                    batch_s[i%4].append([0]+[ch2int[ch] for ch in toks[0]]+[VOCAB_SIZE-1])  #编号0代表GO，编号VOCAB_SIZE-1代表<EOS>
                    # fin中每四行是一首诗，batch_s中每一个列表存储batch_size首诗的同一行的每一个字符外加开始符结束符在字库中的编号，这可不是word embedding
                    batch_kw[i%4].append([ch2int[ch] for ch in toks[1]])  # 该行诗句的关键字索引编号，之后需要通过tf.embedding_lookup来生成词向量
            if 0 == len(batch_s[0]):
                break
            else:
