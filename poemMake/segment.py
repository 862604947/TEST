...
class Segmenter:
    ...
    def segment(self, sentence):  # sentence是一句诗句
        if 0 == len(sentence):
            return []  # 该诗句没有词
        elif 1 == len(sentence):
            return [sentence]  # 该句只有一个字
        elif 2 == len(sentence):
            return [sentence] if sentence in self._sxhy_dict else jieba.lcut(sentence, HMM=True)  # 该句只有两个字
        else:
            segs = []
